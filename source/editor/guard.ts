...
/**
 * The structural guard on `/editor` proposals.
 *
 * The editor is allowed to correct spelling and grammar and nothing else. That
...
export function guardCorrection(existing: string, proposed: string): GuardVerdict {
	...
	// as "one line, one word added". The editor corrects; it never authors.
	if (existing.trim() === '') {
		return refuse('no existing file — the editor never creates a file');
	}
	...
	if (before.front !== after.front) {
		return refuse('frontmatter changed — the editor may not touch data');
	}
