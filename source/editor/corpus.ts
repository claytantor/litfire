...
/**
 * Full-text budget for `buildEditorContext`, matching `DEFAULT_BUDGET` in
 ...
 * instead. Falling back keeps every kind navigable in the map — an editor
...
/**
 * Builds the whole-vault map the editor always sees.
 *
 ...
 * same reason `interview/grounding.ts` withholds it — an editor that cites a
...
export async function buildCorpusMap(root: string, project: Project): Promise<CorpusMap> {
	...
	return {
		...
		// the editor discusses what is still unresolved, not the full history.
 ...
 * would let a scaffold name reach the editor, which is the failure the
 ...
 * editor turn, so it must never grow with a file's size — full text is what
 * `selectRelevant`/`buildEditorContext` are for.
 */
 ...
 * scenes to hand the editor when nothing actually matched would let it answer
...
/**
 * Composes the string injected into the editor's prompt: the map (always
...
export async function buildEditorContext(
	...
	// The editor answers questions about consistency, so the computed
