...
import type {EditorRole, EditorTurn} from '../editor/types.js';
import {
	contentWidth,
	rowsFor,
	splitRow,
...
type Props = {
	readonly turns: readonly EditorTurn[];
	...
	/** esc — cancels an in-flight reply, or leaves the editor when idle. */
	readonly onCancel: () => void;
	/**
	 * Who the assistant side of the conversation is. Defaults to the editor.
	 *
	 * Names every user-visible mention of them — the heading, the turn labels,
	 * the placeholder, the busy line — because a screen that greets you as one
	 * agent while you are talking to another is worse than a screen with no name
	 * on it at all. `/architect` and `/editor` do different jobs and the author
	 * has to be able to tell which one they have open.
	 */
	readonly speaker?: string;
};

/**
 * A `Record` keyed by the role rather than a ternary: if `EditorRole` ever grows
 * a third member this stops compiling, instead of silently rendering the new
 * role in the editor's colours.
 */
const SPEAKER: Record<EditorRole, {readonly label: string; readonly color: string}> = {
	author: {label: `${theme.symbol.user} you`, color: theme.color.user},
	editor: {label: `${theme.symbol.assistant} editor`, color: theme.color.assistant},
};

/** The same roles, with the assistant renamed for whoever is running the screen. */
function speakersFor(
	speaker: string | undefined,
): Record<EditorRole, {readonly label: string; readonly color: string}> {
...
function turnLines(
	...
	text: string,
	width: number,
	speakers: Record<EditorRole, {readonly label: string; readonly color: string}>,
): Line[] {
 ...
 * scroll-back the author needs to re-read an editor's earlier reasoning while a
...
export function EditorScreen({
	 ...
	 * one row further behind with every line the editor streams, and correcting
	...
	const partial = useMemo(
		() =>
			streaming === undefined ? [] : turnLines('editor', streaming, width, speakers),
	...
	const header = splitRow(
		'editor',
		`↑ ${String(lines.length)} back · ↓ to follow`,
		width,
	);
	...
	return (
		<Box flexDirection="column">
			...
			>
				...
				>
					...
					{/*
						...
						idle but is not is how someone concludes the editor has stalled.
					*/}
					{up > 0 ? (
			...
			{/*
				...
				not shift by one every time the editor starts or stops working.
			*/}
			<Box paddingX={1}>
				{busy && (
			...
			*/}
			<Composer
				...
				busyLabel={`the ${name} is replying…`}
			/>
