...
describe('buildCorpusMap', () => {
	 ...
	 * placeholders so the vault opens as a connected Obsidian graph. An editor
...
describe('buildEditorContext', () => {
	it('always includes the map even when the full-text budget is 0', async () => {
		...
		const project = await computeProject(root);
		const context = await buildEditorContext(root, project, 'Tell me about nyx', {
			budget: 0,
		});
	...
	it('includes the full text of a selected file when the budget allows', async () => {
		...
		const project = await computeProject(root);
		const context = await buildEditorContext(root, project, 'sit-901 harbor duel');
