...
function mount(over: Partial<typeof base> = {}) {
	...
	const ui = render(<EditorScreen {...props} onSubmit={onSubmit} onCancel={onCancel} />);

	/** Re-renders with a fresh slice of parent state, as the session would. */
	const show = (next: Partial<typeof base>) => {
		ui.rerender(
			<EditorScreen {...props} {...next} onSubmit={onSubmit} onCancel={onCancel} />,
		);
	};
...
const many: readonly EditorTurn[] = Array.from({length: 30}, (_unused, index) => ({
...
describe('EditorScreen', () => {
	it('renders both sides of the conversation, each with its own prefix', async () => {
		const {lastFrame} = mount({
			turns: [
				...
				{role: 'editor', text: 'Not on the page.'},
			],
		});
		await flush();
	...
	it('renders the streaming partial as it arrives', async () => {
		...
		const frame = lastFrame() ?? '';
		// Attributed to the editor while still in flight, not once it lands.
		expect(frame).toContain('⏺ editor');
	...
	it('submits a line and clears the draft', async () => {
		...
		expect(lastFrame()).toContain('ask the editor…');
	});
	...
	it('follows new output while pinned to the tail', async () => {
		...
		show({turns: [...many, {role: 'editor', text: 'turn-30'}]});
		await flush();
	...
	it('wraps a long reply instead of truncating it', async () => {
		...
		const {lastFrame} = mount({turns: [{role: 'editor', text: reply}]});
		await flush();
...
describe('who the author is talking to', () => {
	const turns: readonly EditorTurn[] = [
		{role: 'author', text: 'split the lathe'},
		{role: 'editor', text: 'two systems, then'},
	];

	it('says editor by default, everywhere it says anything', async () => {
		const ui = mount({turns});
		await flush();
